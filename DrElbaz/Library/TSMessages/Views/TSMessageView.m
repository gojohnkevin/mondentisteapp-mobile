...
#define TSMessageImagePadding 5.0
...
- (CGFloat)updateHeightOfMessageView
{
    ...
    {
        self.contentLabel.frame = CGRectMake(self.textSpaceLeft,5.0,
                                             screenWidth - TSMessageViewPadding - self.textSpaceLeft - self.textSpaceRight,
                                             0.0);
    ...
    currentHeight += TSMessageImagePadding;
    
    if (self.iconImageView)
    {
        // Check if that makes the popup larger (height)
        if (self.iconImageView.frame.origin.y + self.iconImageView.frame.size.height + TSMessageImagePadding > currentHeight)
        {
            currentHeight = self.iconImageView.frame.origin.y + self.iconImageView.frame.size.height + TSMessageImagePadding;
        }
        else
        {
            // z-align
